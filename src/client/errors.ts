...
/** Hostnames that are official Kimi endpoints (used to gate the create-key link). */
const OFFICIAL_API_HOSTS = ['api.moonshot.ai', 'api.moonshot.cn'];
...
const SET_API_KEY_COMMAND = 'command:kimi-copilot.setApiKey';

/** VS Code command URI that reveals the extension log output. */
const SHOW_LOGS_COMMAND = 'command:kimi-copilot.showLogs';

type RequestErrorKind = 'http' | 'network' | 'unknown';

interface KimiRequestErrorOptions {
	message: string;
...
/** A Kimi request failure carrying both a user-facing summary and diagnostics. */
export class KimiRequestError extends Error {
	...
	constructor(options: KimiRequestErrorOptions) {
...
/** Build a `KimiRequestError` from a non-OK HTTP response. */
export async function createHttpError(
	response: Response,
	context: { baseUrl: string },
): Promise<KimiRequestError> {
	...
	return new KimiRequestError({
		message: `Kimi API request failed with HTTP ${response.status}`,
		userSummary,
		kind: 'http',
		baseUrl,
...
/** Categorize a thrown value into a `KimiRequestError`, or return it unchanged. */
export function normalizeRequestError(
	error: unknown,
	context: { baseUrl: string },
): KimiRequestError | Error {
	...
	if (!(error instanceof Error)) {
		...
		return new KimiRequestError({
			message: `Kimi request failed with a non-Error value: ${value}`,
	...
	const enhanced = new KimiRequestError({
		message: code
			? `Kimi request failed due to network error ${code}`
			: 'Kimi request failed due to a network error',
		userSummary,
		kind: 'network',
		baseUrl: context.baseUrl,
		code,
...
export function createUserFacingError(error: unknown): Error {
	const message =
		error instanceof KimiRequestError
...
function getErrorActions(error: KimiRequestError): ErrorAction[] {
