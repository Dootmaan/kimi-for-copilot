 ...
 *
 * Non-retryable HTTP responses and exhausted retries throw a `KimiRequestError`
...
export async function fetchChatCompletionWithRetry(
	...
	for (let attempt = 0; ; attempt++) {
		...
		logger.warn(
			`Kimi API returned HTTP ${response.status} on attempt ${attempt + 1}/${maxAttempts}; retrying in ${delay}ms`,
		);
