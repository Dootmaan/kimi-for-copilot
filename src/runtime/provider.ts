...
import { resolveUsageHost, resolveUsagePath } from '../endpoint';
import { logger } from '../logger';
import { KimiChatProvider } from '../provider';
import { UsageStatusBar } from './usage-bar';

export async function registerProvider(context: vscode.ExtensionContext): Promise<KimiChatProvider> {
	const authManager = new AuthManager(context);
	const provider = new KimiChatProvider(context, authManager);
	const usageClient = new UsageClient(resolveUsageHost, resolveUsagePath);
